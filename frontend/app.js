...
function getCabinetMetricHTML(startYear, endYear, dataStart, dataEnd, meta) {
    ...
    // Calculate Average
    const yearsList = statsData.filter(d => d.year >= startYear && d.year <= endYear);
    const sumVal = yearsList.reduce((acc, curr) => acc + (curr[activeIndicator] || 0), 0);
    const avgVal = yearsList.length > 0 ? sumVal / yearsList.length : 0;
    ...
    const startLbl = startYear === endYear ? `Hodnota:` : `Na začiatku (${startYear}):`;
    ...
    return `
        <div class="cabinet-metric-value">
            ...
            <span class="val-num text-highlight">${formattedStart}</span>
        </div>
        ${endRowHTML}
        ${avgRowHTML}
    `;
...
function updateDashboard() {
    ...
    if (isHist) {
        ...
        // Victory score calculations across all 13 indicators
        let victoriesA = 0;
        let victoriesB = 0;
        Object.keys(indicatorsMeta).forEach(key => {
            ...
            if (avgA !== avgB) {
                const isBetterA = meta.higherIsBetter ? (avgA > avgB) : (avgA < avgB);
                if (isBetterA) victoriesA++;
                else victoriesB++;
            }
        });
        ...
        // Calculate averages for comparison and styling
        const listA = statsData.filter(d => d.year >= startYearA && d.year <= endYearA);
        const avgA = listA.length > 0 ? (listA.reduce((sum, curr) => sum + (curr[activeIndicator] || 0), 0) / listA.length) : 0;
        
        const listB = statsData.filter(d => d.year >= startYearB && d.year <= endYearB);
        const avgB = listB.length > 0 ? (listB.reduce((sum, curr) => sum + (curr[activeIndicator] || 0), 0) / listB.length) : 0;
        ...
        if (deltaContainer) {
            ...
            deltaContainer.innerHTML = `
                <span class="delta-label-desc">Porovnanie priemerných hodnôt (Obdobie B vs Obdobie A):</span>
                <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
                    <span class="delta-badge-large ${badgeClass}">${deltaText}</span>
                    <span class="delta-label-desc">(${directionText})</span>
                </div>
