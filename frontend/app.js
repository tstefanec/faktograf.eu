...
async function fetchMemes() {
    ...
    try {
        ...
        if (memes.length === 0) {
            container.innerHTML = `
                <div style="font-size: 11px; color: var(--color-text-secondary); text-align: center; padding: 16px; border: 1px dashed rgba(255,255,255,0.08); border-radius: 6px; background: rgba(9,10,12,0.2);">
                    <i class="fa-solid fa-images" style="margin-bottom: 6px; display: block; font-size: 18px; opacity: 0.6; color: var(--color-beige);"></i>
            ...
            html += `
                <div class="meme-item-wrapper" onclick="openMemeLightbox('${url}')" style="cursor: zoom-in; overflow: hidden; border-radius: 8px; border: 1.5px solid rgba(229, 211, 179, 0.25); background: rgba(9,10,12,0.5); transition: all 0.3s cubic-bezier(0.25, 0.8, 0.25, 1); position: relative;">
                    ...
                    <div class="meme-zoom-overlay" style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: rgba(9,10,12,0.45); display: flex; align-items: center; justify-content: center; opacity: 0; transition: opacity 0.3s ease;">
                        <i class="fa-solid fa-magnifying-glass-plus" style="color: var(--color-beige); font-size: 20px;"></i>
                    </div>
                </div>
            `;
        });
        ...
        container.innerHTML = `
            <div style="font-size: 11px; color: var(--color-red); text-align: center; padding: 12px;">
                <i class="fa-solid fa-triangle-exclamation"></i> Chyba pri načítaní memečiek.
            </div>
        `;
    }
}
